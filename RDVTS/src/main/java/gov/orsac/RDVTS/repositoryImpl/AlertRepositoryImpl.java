...
public class AlertRepositoryImpl {
    ...
    public Integer getTotalAlertToday() {
        ...
        String qry = " select distinct id, imei,alert_type_id from rdvts_oltp.alert_data where imei in (select imei_no_1 from rdvts_oltp.device_m where id in " +
                ...
                " (select activity_id from rdvts_oltp.activity_work_mapping where work_id in (select id from rdvts_oltp.work_m))))) " +
